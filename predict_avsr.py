...
def calculate_cosine_similarity(audio_feat, video_feat):
    # 将二维特征向量裁剪为相同的长度
    if not audio_feat.shape[0] == video_feat.shape[0]:
        return 0.0
...
def main(cfg):
    ...
    model = E2E(len(token_list), cfg.model.audiovisual_backbone).to(device)
    
    model.load_state_dict(torch.load(cfg.ckpt_path, map_location=lambda storage, loc: storage))
    ...
    for i, (fn, length, label, audio_label, video_label) in enumerate(tqdm(fns)):
        ...
        with torch.no_grad():
            audio_enc_feat = model.aux_encoder(audio.unsqueeze(0).to(device), None)[0].squeeze(0)
            video_enc_feat = model.encoder(video.unsqueeze(0).to(device), None)[0].squeeze(0)
